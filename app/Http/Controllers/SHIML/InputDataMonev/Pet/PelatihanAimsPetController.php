...
use App\Http\Requests\SHIML\Pet\PelatihanAimsPetRequest;
use App\Models\SHIML\Pet\PelatihanAimsPet;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\DB;

class PelatihanAimsPetController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return view('SHIML.InputDataMonev.Pet.PelatihanAimsPet', compact('tabs'));
    }

    public function store(PelatihanAimsPetRequest $request)
    {
        $validated = $request->validated();
        $TargetPLO = PelatihanAimsPet::create($validated);
    ...
    {
        $TargetPLO = PelatihanAimsPet::select('*')
                 ->addSelect(DB::raw("TRY_CONVERT(DATE, CONCAT('01-', periode), 120) as periode_date"))
            ->orderBy('periode_date', 'asc')
            ->get();
        return response()->json($TargetPLO);
    }
    public function update(PelatihanAimsPetRequest $request, $id)
    {
        $progress = PelatihanAimsPet::findOrFail($id);
    ...
    {
        $target = PelatihanAimsPet::findOrFail($id);
        $target->delete();
