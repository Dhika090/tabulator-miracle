...
use App\Http\Requests\SHIML\Pet\AssetBreakdownPetRequest;
use App\Models\SHIML\Pet\AssetBreakdownPet;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\DB;

class AssetBreakdownPetController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return view('SHIML.InputDataMonev.Pet.AssetBreakdownPet', compact('tabs'));
    }

    public function store(AssetBreakdownPetRequest $request)
    {
        $validated = $request->validated();
        $TargetPLO = AssetBreakdownPet::create($validated);
    ...
    {
        $TargetPLO = AssetBreakdownPet::select('*')
                 ->addSelect(DB::raw("TRY_CONVERT(DATE, CONCAT('01-', periode), 120) as periode_date"))
            ->orderBy('periode_date', 'asc')
            ->get();
        return response()->json($TargetPLO);
    }
    public function update(AssetBreakdownPetRequest $request, $id)
    {
        $progress = AssetBreakdownPet::findOrFail($id);
    ...
    {
        $target = AssetBreakdownPet::findOrFail($id);
        $target->delete();
