...
use App\Http\Requests\SHIML\Pet\KondisiVacantAimsPetRequest;
use App\Models\SHIML\Pet\KondisiVacantaimsPet;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\DB;

class KondisiVacantAimsPetController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return view('SHIML.InputDataMonev.Pet.KondisiVacantaimsPet', compact('tabs'));
    }

    public function store(KondisiVacantAimsPetRequest $request)
    {
        $validated = $request->validated();
        $TargetPLO = KondisiVacantaimsPet::create($validated);
    ...
    {
        $TargetPLO = KondisiVacantaimsPet::select('*')
                 ->addSelect(DB::raw("TRY_CONVERT(DATE, CONCAT('01-', periode), 120) as periode_date"))
            ->orderBy('periode_date', 'asc')
            ->get();
        return response()->json($TargetPLO);
    }
    public function update(KondisiVacantAimsPetRequest $request, $id)
    {
        $progress = KondisiVacantaimsPet::findOrFail($id);
    ...
    {
        $target = KondisiVacantaimsPet::findOrFail($id);
        $target->delete();
