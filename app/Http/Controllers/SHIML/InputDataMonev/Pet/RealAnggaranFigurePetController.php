...
use App\Http\Requests\SHIML\Pet\RealAnggaranFigurePetRequest;
use App\Models\SHIML\Pet\RealAnggaranFigurePet;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\DB;

class RealAnggaranFigurePetController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return view('SHIML.InputDataMonev.Pet.RealAnggaranFigurePet', compact('tabs'));
    }

    public function store(RealAnggaranFigurePetRequest $request)
    {
        $validated = $request->validated();
        $TargetPLO = RealAnggaranFigurePet::create($validated);
    ...
    {
        $TargetPLO = RealAnggaranFigurePet::select('*')
                 ->addSelect(DB::raw("TRY_CONVERT(DATE, CONCAT('01-', periode), 120) as periode_date"))
            ->orderBy('periode_date', 'asc')
            ->get();
        return response()->json($TargetPLO);
    }
    public function update(RealAnggaranFigurePetRequest $request, $id)
    {
        $progress = RealAnggaranFigurePet::findOrFail($id);
    ...
    {
        $target = RealAnggaranFigurePet::findOrFail($id);
        $target->delete();
