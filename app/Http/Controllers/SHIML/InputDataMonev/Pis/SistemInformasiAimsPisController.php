...
use App\Http\Requests\SHIML\Pis\SistemInformasiAimsPisRequest;
use App\Models\SHIML\Pis\SistemInformasiAimsPis;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\DB;

class SistemInformasiAimsPisController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return view('SHIML.InputDataMonev.Pis.SistemInformasiAimsPis', compact('tabs'));
    }

    public function store(SistemInformasiAimsPisRequest $request)
    {
        $validated = $request->validated();
        $TargetPLO = SistemInformasiAimsPis::create($validated);
    ...
    {
        $TargetPLO = SistemInformasiAimsPis::select('*')
                 ->addSelect(DB::raw("TRY_CONVERT(DATE, CONCAT('01-', periode), 120) as periode_date"))
            ->orderBy('periode_date', 'asc')
            ->get();
        return response()->json($TargetPLO);
    }
    public function update(SistemInformasiAimsPisRequest $request, $id)
    {
        $progress = SistemInformasiAimsPis::findOrFail($id);
    ...
    {
        $target = SistemInformasiAimsPis::findOrFail($id);
        $target->delete();
