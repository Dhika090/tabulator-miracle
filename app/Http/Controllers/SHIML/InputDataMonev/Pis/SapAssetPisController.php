...
use App\Http\Requests\SHIML\Pis\SapAssetPisRequest;
use App\Models\SHIML\Pis\SapAssetPis;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\DB;

class SapAssetPisController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return view('SHIML.InputDataMonev.Pis.SapAssetPis', compact('tabs'));
    }

    public function store(SapAssetPisRequest $request)
    {
        $validated = $request->validated();
        $TargetPLO = SapAssetPis::create($validated);
    ...
    {
        $TargetPLO = SapAssetPis::select('*')
                 ->addSelect(DB::raw("TRY_CONVERT(DATE, CONCAT('01-', periode), 120) as periode_date"))
            ->orderBy('periode_date', 'asc')
            ->get();
        return response()->json($TargetPLO);
    }
    public function update(SapAssetPisRequest $request, $id)
    {
        $progress = SapAssetPis::findOrFail($id);
    ...
    {
        $target = SapAssetPis::findOrFail($id);
        $target->delete();
