...
use App\Http\Requests\SHIML\Pis\StatusAssetAiPisRequest;
use App\Models\SHIML\Pis\StatusAssetAiPis;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\DB;

class StatusAssetAiPisController extends Controller
{

    public function index(Request $request)
    {
        if ($request->wantsJson()) {
            $TargetPLO = StatusAssetAiPis::all();
        ...
        return view('SHIML.InputDataMonev.Pis.StatusAssetAiPis', compact('tabs', 'companies'));
    }

    public function store(StatusAssetAiPisRequest $request)
    {
        $validated = $request->validated();
        $TargetPLO = StatusAssetAiPis::create($validated);
    ...
    {
        $TargetPLO = StatusAssetAiPis::select('*')
                 ->addSelect(DB::raw("TRY_CONVERT(DATE, CONCAT('01-', periode), 120) as periode_date"))
            ->orderBy('periode_date', 'asc')
            ->get();
        return response()->json($TargetPLO);
    }
    public function update(StatusAssetAiPisRequest $request, $id)
    {
        $progress = StatusAssetAiPis::findOrFail($id);
    ...
    {
        $target = StatusAssetAiPis::findOrFail($id);
        $target->delete();
