...
use App\Http\Requests\SHIML\Pis\AvailabilityPisRequest;
use App\Models\SHIML\Pis\AvailabilityPis;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\DB;

class AvailabilityPisController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return view('SHIML.InputDataMonev.Pis.AvailabilityPis', compact('tabs'));
    }

    public function store(AvailabilityPisRequest $request)
    {
        $validated = $request->validated();
        $TargetPLO = AvailabilityPis::create($validated);
    ...
    {
        $TargetPLO = AvailabilityPis::select('*')
                 ->addSelect(DB::raw("TRY_CONVERT(DATE, CONCAT('01-', periode), 120) as periode_date"))
            ->orderBy('periode_date', 'asc')
            ->get();
        return response()->json($TargetPLO);
    }
    public function update(AvailabilityPisRequest $request, $id)
    {
        $progress = AvailabilityPis::findOrFail($id);
    ...
    {
        $target = AvailabilityPis::findOrFail($id);
        $target->delete();
