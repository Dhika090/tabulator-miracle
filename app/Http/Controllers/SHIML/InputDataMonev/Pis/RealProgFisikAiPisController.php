...
use App\Http\Requests\SHIML\Pis\RealProgFisikAiPisRequest;
use App\Models\SHIML\Pis\RealProgFisikAiPis;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\DB;

class RealProgFisikAiPisController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return view('SHIML.InputDataMonev.Pis.RealProgFisikAiPis', compact('tabs'));
    }

    public function store(RealProgFisikAiPisRequest $request)
    {
        $validated = $request->validated();
        $TargetPLO = RealProgFisikAiPis::create($validated);
    ...
    {
        $TargetPLO = RealProgFisikAiPis::select('*')
                 ->addSelect(DB::raw("TRY_CONVERT(DATE, CONCAT('01-', periode), 120) as periode_date"))
            ->orderBy('periode_date', 'asc')
            ->get();
        return response()->json($TargetPLO);
    }
    public function update(RealProgFisikAiPisRequest $request, $id)
    {
        $progress = RealProgFisikAiPis::findOrFail($id);
    ...
    {
        $target = RealProgFisikAiPis::findOrFail($id);
        $target->delete();
