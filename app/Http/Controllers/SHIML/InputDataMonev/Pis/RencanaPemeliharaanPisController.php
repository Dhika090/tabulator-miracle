...
use App\Http\Requests\SHIML\Pis\RencanaPemeliharaanPisRequest;
use App\Models\SHIML\Pis\RencanaPemeliharaanPis;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\DB;

class RencanaPemeliharaanPisController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return view('SHIML.InputDataMonev.Pis.RencanaPemeliharaanPis', compact('tabs'));
    }

    public function store(RencanaPemeliharaanPisRequest $request)
    {
        $validated = $request->validated();
        $TargetPLO = RencanaPemeliharaanPis::create($validated);
    ...
    {
        $TargetPLO = RencanaPemeliharaanPis::select('*')
                 ->addSelect(DB::raw("TRY_CONVERT(DATE, CONCAT('01-', periode), 120) as periode_date"))
            ->orderBy('periode_date', 'asc')
            ->get();
        return response()->json($TargetPLO);
    }
    public function update(RencanaPemeliharaanPisRequest $request, $id)
    {
        $progress = RencanaPemeliharaanPis::findOrFail($id);
    ...
    {
        $target = RencanaPemeliharaanPis::findOrFail($id);
        $target->delete();
