...
use App\Http\Requests\SHIML\PTK\PelatihanAimsPtkRequest;
use App\Models\SHIML\PTK\PelatihanAimsPtk;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\DB;

class PelatihanAimsPtkController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return view('SHIML.InputDataMonev.Ptk.PelatihanAimsPtk', compact('tabs'));
    }

    public function store(PelatihanAimsPtkRequest $request)
    {
        $validated = $request->validated();
        $TargetPLO = PelatihanAimsPtk::create($validated);
    ...
    {
        $TargetPLO = PelatihanAimsPtk::select('*')
                 ->addSelect(DB::raw("TRY_CONVERT(DATE, CONCAT('01-', periode), 120) as periode_date"))
            ->orderBy('periode_date', 'asc')
            ->get();
        return response()->json($TargetPLO);
    }
    public function update(PelatihanAimsPtkRequest $request, $id)
    {
        $progress = PelatihanAimsPtk::findOrFail($id);
    ...
    {
        $target = PelatihanAimsPtk::findOrFail($id);
        $target->delete();
