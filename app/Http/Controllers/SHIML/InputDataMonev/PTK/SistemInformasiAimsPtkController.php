...
use App\Http\Requests\SHIML\PTK\SistemInformasiAimsPtkRequest;
use App\Models\SHIML\PTK\SistemInformasiAimsPtk;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\DB;

class SistemInformasiAimsPtkController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return view('SHIML.InputDataMonev.Ptk.SistemInformasiAimsPtk', compact('tabs'));
    }

    public function store(SistemInformasiAimsPtkRequest $request)
    {
        $validated = $request->validated();
        $TargetPLO = SistemInformasiAimsPtk::create($validated);
    ...
    {
        $TargetPLO = SistemInformasiAimsPtk::select('*')
                 ->addSelect(DB::raw("TRY_CONVERT(DATE, CONCAT('01-', periode), 120) as periode_date"))
            ->orderBy('periode_date', 'asc')
            ->get();
        return response()->json($TargetPLO);
    }
    public function update(SistemInformasiAimsPtkRequest $request, $id)
    {
        $progress = SistemInformasiAimsPtk::findOrFail($id);
    ...
    {
        $target = SistemInformasiAimsPtk::findOrFail($id);
        $target->delete();
