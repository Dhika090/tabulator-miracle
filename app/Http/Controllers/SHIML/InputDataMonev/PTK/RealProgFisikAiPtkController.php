...
use App\Http\Requests\SHIML\PTK\RealProgFisikAiPtkRequest;
use App\Models\SHIML\PTK\RealProgFisikAiPtk;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\DB;

class RealProgFisikAiPtkController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return view('SHIML.InputDataMonev.Ptk.RealProgFisikAiPtk', compact('tabs'));
    }

    public function store(RealProgFisikAiPtkRequest $request)
    {
        $validated = $request->validated();
        $TargetPLO = RealProgFisikAiPtk::create($validated);
    ...
    {
        $TargetPLO = RealProgFisikAiPtk::select('*')
                 ->addSelect(DB::raw("TRY_CONVERT(DATE, CONCAT('01-', periode), 120) as periode_date"))
            ->orderBy('periode_date', 'asc')
            ->get();
        return response()->json($TargetPLO);
    }
    public function update(RealProgFisikAiPtkRequest $request, $id)
    {
        $progress = RealProgFisikAiPtk::findOrFail($id);
    ...
    {
        $target = RealProgFisikAiPtk::findOrFail($id);
        $target->delete();
