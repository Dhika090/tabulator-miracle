...
use App\Http\Requests\SHIML\PTK\StatusAssetAiPtkRequest;
use App\Models\SHIML\PTK\StatusAssetAiPtk;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\DB;

class StatusAssetAiPtkController extends Controller
{

    public function index(Request $request)
    {
        if ($request->wantsJson()) {
            $TargetPLO = StatusAssetAiPtk::all();
        ...
        return view('SHIML.InputDataMonev.Ptk.StatusAssetAiPtk', compact('tabs', 'companies'));
    }

    public function store(StatusAssetAiPtkRequest $request)
    {
        $validated = $request->validated();
        $TargetPLO = StatusAssetAiPtk::create($validated);
    ...
    {
        $TargetPLO = StatusAssetAiPtk::select('*')
                 ->addSelect(DB::raw("TRY_CONVERT(DATE, CONCAT('01-', periode), 120) as periode_date"))
            ->orderBy('periode_date', 'asc')
            ->get();
        return response()->json($TargetPLO);
    }
    public function update(StatusAssetAiPtkRequest $request, $id)
    {
        $progress = StatusAssetAiPtk::findOrFail($id);
    ...
    {
        $target = StatusAssetAiPtk::findOrFail($id);
        $target->delete();
