...
use App\Http\Requests\SHIML\PTK\KondisiVacantAimsPtkRequest;
use App\Models\SHIML\PTK\KondisiVacantAimsPtk;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\DB;

class KondisiVacantAimsPtkController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return view('SHIML.InputDataMonev.Ptk.KondisiVacantAimsPtk', compact('tabs'));
    }

    public function store(KondisiVacantAimsPtkRequest $request)
    {
        $validated = $request->validated();
        $TargetPLO = KondisiVacantAimsPtk::create($validated);
    ...
    {
        $TargetPLO = KondisiVacantAimsPtk::select('*')
                 ->addSelect(DB::raw("TRY_CONVERT(DATE, CONCAT('01-', periode), 120) as periode_date"))
            ->orderBy('periode_date', 'asc')
            ->get();
        return response()->json($TargetPLO);
    }
    public function update(KondisiVacantAimsPtkRequest $request, $id)
    {
        $progress = KondisiVacantAimsPtk::findOrFail($id);
    ...
    {
        $target = KondisiVacantAimsPtk::findOrFail($id);
        $target->delete();
