...
use App\Http\Requests\SHPNRE\Jawa1Power\StatusAssetAiJawa1PowerRequest;
use App\Models\SHPNRE\Jawa1Power\StatusAssetAiJawa1Power;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\DB;

class StatusAssetAiJawa1PowerController extends Controller
{

    public function index(Request $request)
    {
        if ($request->wantsJson()) {
            $TargetPLO = StatusAssetAiJawa1Power::all();
        ...
        return view('SHPNRE.InputDataMonev.Jawa1Power.StatusAssetAiJawa1Power', compact('tabs','companies'));
    }


    public function store(StatusAssetAiJawa1PowerRequest $request)
    {
        $validated = $request->validated();
        $TargetPLO = StatusAssetAiJawa1Power::create($validated);
    ...
    {
        $TargetPLO = StatusAssetAiJawa1Power::select('*')
            ->addSelect(DB::raw("
    ...
    public function update(StatusAssetAiJawa1PowerRequest $request, $id)
    {
        $progress = StatusAssetAiJawa1Power::findOrFail($id);
    ...
    {
        $target = StatusAssetAiJawa1Power::findOrFail($id);
        $target->delete();
