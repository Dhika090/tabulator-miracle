...
class AvailabilitySOR3Controller extends Controller
{
    public function index(Request $request)
    {
        ...
        return view('SHG.InputDataMonev.PgnSor3.AvailabilitySOR3', compact('tabs'));
    }

    public function store(AvailabilitySOR3Request $request)
    {
        $validated = $request->validated();
        $TargetPLO = AvailabilitySOR3::create($validated);
    ...
    {
        $progress = AvailabilitySOR3::findOrFail($id);
    ...
    {
        $target = AvailabilitySOR3::findOrFail($id);
        $target->delete();
