...
use App\Http\Requests\SHG\PgnSor3\StatusPloSOR3Request;
use App\Models\SHG\PgnSor3\StatusPloSOR3;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\DB;

class StatusPloSOR3controller extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return view('SHG.InputDataMonev.PgnSor3.StatusPloSOR3', compact('tabs'));
    }

    public function store(StatusPloSOR3Request $request)
    {
        $validated = $request->validated();
        $TargetPLO = StatusPloSOR3::create($validated);
    ...
    {
        $TargetPLO = StatusPloSOR3::select('*')
             ->addSelect(DB::raw("TRY_CONVERT(DATE, CONCAT('01-', periode), 120) as periode_date"))
            ->orderBy('periode_date', 'asc')
            ->get();
        return response()->json($TargetPLO);
    }
    public function update(StatusPloSOR3Request $request, $id)
    {
        $progress = StatusPloSOR3::findOrFail($id);
    ...
    {
        $target = StatusPloSOR3::findOrFail($id);
        $target->delete();
