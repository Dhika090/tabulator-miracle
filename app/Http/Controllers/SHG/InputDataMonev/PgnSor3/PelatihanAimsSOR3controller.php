...
class PelatihanAimsSOR3controller extends Controller
{
    public function index(Request $request)
    {
     ...
     $tabs = [
        ...
        return view('SHG.InputDataMonev.PgnSor3.PelatihanAimsSOR3', compact('tabs'));
    }

    public function store(PelatihanAimsSOR3Request $request)
    {
        $validated = $request->validated();
        $TargetPLO = PelatihanAimsSOR3::create($validated);
    ...
    {
        $progress = PelatihanAimsSOR3::findOrFail($id);
    ...
    {
        $target = PelatihanAimsSOR3::findOrFail($id);
        $target->delete();
