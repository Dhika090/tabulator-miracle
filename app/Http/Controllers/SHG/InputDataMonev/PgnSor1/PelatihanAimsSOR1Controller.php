...
use App\Http\Requests\SHG\PgnSor1\PelatihanAimsSOR1Request;
use App\Models\SHG\PgnSor1\PelatihanAimsSOR1;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\DB;

class PelatihanAimsSOR1Controller extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return view('SHG.InputDataMonev.PgnSor1.PelatihanAimsSOR1', compact('tabs'));
    }

    public function store(PelatihanAimsSOR1Request $request)
    {
        $validated = $request->validated();
        $TargetPLO = PelatihanAimsSOR1::create($validated);
    ...
    {
        $TargetPLO = PelatihanAimsSOR1::select('*')
             ->addSelect(DB::raw("TRY_CONVERT(DATE, CONCAT('01-', periode), 120) as periode_date"))
            ->orderBy('periode_date', 'asc')
            ->get();
        return response()->json($TargetPLO);
    }
    public function update(PelatihanAimsSOR1Request $request, $id)
    {
        $progress = PelatihanAimsSOR1::findOrFail($id);
    ...
    {
        $target = PelatihanAimsSOR1::findOrFail($id);
        $target->delete();
