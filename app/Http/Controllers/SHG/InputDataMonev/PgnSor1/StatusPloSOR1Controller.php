...
use App\Http\Requests\SHG\PgnSor1\StatusPloSOR1Request;
use App\Models\SHG\PgnSor1\StatusPloSOR1;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\DB;

class StatusPloSOR1Controller extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return view('SHG.InputDataMonev.PgnSor1.StatusPloSOR1', compact('tabs'));
    }

    public function store(StatusPloSOR1Request $request)
    {
        $validated = $request->validated();
        $TargetPLO = StatusPloSOR1::create($validated);
    ...
    {
        $TargetPLO = StatusPloSOR1::select('*')
             ->addSelect(DB::raw("TRY_CONVERT(DATE, CONCAT('01-', periode), 120) as periode_date"))
            ->orderBy('periode_date', 'asc')
            ->get();
        return response()->json($TargetPLO);
    }
    public function update(StatusPloSOR1Request $request, $id)
    {
        $progress = StatusPloSOR1::findOrFail($id);
    ...
    {
        $target = StatusPloSOR1::findOrFail($id);
        $target->delete();
