...
use App\Http\Requests\SHG\NusantaraRegas\AssetBreakdownNRReuqest;
use App\Models\SHG\NusantaraRegas\AssetBreakdownNR;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\DB;

class AssetBreakdownNRController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return view('SHG.InputDataMonev.NusantaraRegas.AssetBreakdownNR', compact('tabs'));
    }

    public function store(AssetBreakdownNRReuqest $request)
    {
        $validated = $request->validated();
        $TargetPLO = AssetBreakdownNR::create($validated);
    ...
    {
        $TargetPLO = AssetBreakdownNR::select('*')
             ->addSelect(DB::raw("TRY_CONVERT(DATE, CONCAT('01-', periode), 120) as periode_date"))
            ->orderBy('periode_date', 'asc')
            ->get();
        return response()->json($TargetPLO);
    }
    public function update(AssetBreakdownNRReuqest $request, $id)
    {
        $progress = AssetBreakdownNR::findOrFail($id);
    ...
    {
        $target = AssetBreakdownNR::findOrFail($id);
        $target->delete();
