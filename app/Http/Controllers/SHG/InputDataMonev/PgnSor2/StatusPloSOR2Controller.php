...
use App\Http\Requests\SHG\PgnSor2\StatusPloSOR2Request;
use App\Models\SHG\PgnSor2\StatusPloSOR2;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\DB;

class StatusPloSOR2Controller extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return view('SHG.InputDataMonev.PgnSor2.StatusPloSOR2', compact('tabs'));
    }

    public function store(StatusPloSOR2Request $request)
    {
        $validated = $request->validated();
        $TargetPLO = StatusPloSOR2::create($validated);
    ...
    {
        $TargetPLO = StatusPloSOR2::select('*')
            ->addSelect(DB::raw("
    ...
    public function update(StatusPloSOR2Request $request, $id)
    {
        $progress = StatusPloSOR2::findOrFail($id);
    ...
    {
        $target = StatusPloSOR2::findOrFail($id);
        $target->delete();
