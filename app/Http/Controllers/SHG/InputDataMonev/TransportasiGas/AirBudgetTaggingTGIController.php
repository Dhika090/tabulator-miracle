...
use App\Http\Requests\SHG\TransportasiGas\AirBudgetTaggingTGIRequest;
use App\Models\SHG\TransportasiGas\AirBudgetTaggingTGI;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\DB;

class AirBudgetTaggingTGIController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return view('SHG.InputDataMonev.TransportasiGas.AirBudgetTaggingTGI', compact('tabs'));
    }

    public function store(AirBudgetTaggingTGIRequest $request)
    {
        $validated = $request->validated();
        $TargetPLO = AirBudgetTaggingTGI::create($validated);
    ...
    {
       $TargetPLO = AirBudgetTaggingTGI::select('*')
             ->addSelect(DB::raw("TRY_CONVERT(DATE, CONCAT('01-', periode), 120) as periode_date"))
            ->orderBy('periode_date', 'asc')
            ->get();
        return response()->json($TargetPLO);
    }
    public function update(AirBudgetTaggingTGIRequest $request, $id)
    {
        $progress = AirBudgetTaggingTGI::findOrFail($id);
    ...
    {
        $target = AirBudgetTaggingTGI::findOrFail($id);
        $target->delete();
