...
use App\Http\Requests\SHG\TransportasiGas\RencanaPemeliharaanTGIRequest;
use App\Models\SHG\TransportasiGas\RencanaPemeliharaanTGI;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\DB;

class RencanaPemeliharaanTGIController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return view('SHG.InputDataMonev.TransportasiGas.RencanaPemeliharaanTGI', compact('tabs'));
    }

    public function store(RencanaPemeliharaanTGIRequest $request)
    {
        $validated = $request->validated();
        $TargetPLO = RencanaPemeliharaanTGI::create($validated);
    ...
    {
        $TargetPLO = RencanaPemeliharaanTGI::select('*')
             ->addSelect(DB::raw("TRY_CONVERT(DATE, CONCAT('01-', periode), 120) as periode_date"))
            ->orderBy('periode_date', 'asc')
            ->get();
        return response()->json($TargetPLO);
    }
    public function update(RencanaPemeliharaanTGIRequest $request, $id)
    {
        $progress = RencanaPemeliharaanTGI::findOrFail($id);
    ...
    {
        $target = RencanaPemeliharaanTGI::findOrFail($id);
        $target->delete();
