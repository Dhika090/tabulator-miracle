...
use App\Http\Requests\SHG\PgnOmm\KondisiVacantAimsOmmRequest;
use App\Models\SHG\PgnOmm\KondisiVacantAimsOmm;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\DB;

class KondisiVacantAimsOmmController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return view('SHG.InputDataMonev.PgnOmm.KondisiVacantFungsiAimsOMM', compact('tabs'));
    }

    public function store(KondisiVacantAimsOmmRequest $request)
    {
        $validated = $request->validated();
        $TargetPLO = KondisiVacantAimsOmm::create($validated);
    ...
    {
        $TargetPLO = KondisiVacantAimsOmm::select('*')
            ->addSelect(DB::raw("TRY_CONVERT(DATE, CONCAT('01-', periode), 120) as periode_date"))
            ->orderBy('periode_date', 'asc')
            ->get();
        return response()->json($TargetPLO);
    }
    public function update(KondisiVacantAimsOmmRequest $request, $id)
    {
        $progress = KondisiVacantAimsOmm::findOrFail($id);
    ...
    {
        $target = KondisiVacantAimsOmm::findOrFail($id);
        $target->delete();
