...
use App\Http\Requests\SHG\PgnOmm\SapAssetOmmRequest;
use App\Models\SHG\PgnOmm\SapAssetOmm;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\DB;

class SapAssetOmmController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return view('SHG.InputDataMonev.PgnOmm.SapAssetOMM', compact('tabs'));
    }

    public function store(SapAssetOmmRequest $request)
    {
        $validated = $request->validated();
        $TargetPLO = SapAssetOmm::create($validated);
    ...
    {
        $TargetPLO = SapAssetOmm::select('*')
             ->addSelect(DB::raw("TRY_CONVERT(DATE, CONCAT('01-', periode), 120) as periode_date"))
            ->orderBy('periode_date', 'asc')
            ->get();
        return response()->json($TargetPLO);
    }
    public function update(SapAssetOmmRequest $request, $id)
    {
        $progress = SapAssetOmm::findOrFail($id);
    ...
    {
        $target = SapAssetOmm::findOrFail($id);
        $target->delete();
